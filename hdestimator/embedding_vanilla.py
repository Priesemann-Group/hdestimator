...
try:
    ...
    pyximport.install()
    from . import embedding_cython as fast_emb
except Exception as e:
    log.warning(e)
    FAST_EMBEDDING_AVAILABLE = False
    print(
        "Error importing Cython fast embedding module. Continuing with slow Python"
    ...
    for past_range_T in embedding_past_range_set:
        for number_of_bins_d in embedding_number_of_bins_set:
            if not isinstance(number_of_bins_d, int) or number_of_bins_d < 1:
                print(
                    "Error: numer of bins {} is not a positive integer. Skipping.".format(
                        number_of_bins_d
                    ),
                    file=stderr,
                    flush=True,
...
def get_raw_symbols(spike_times, embedding, first_bin_size, embedding_step_size):
    """
...
def get_symbol_counts(spike_times, embedding, embedding_step_size):
    ...
    if FAST_EMBEDDING_AVAILABLE:
        return Counter(
            fast_emb.get_symbol_counts(spike_times, embedding, embedding_step_size)
        )

    past_range_T, number_of_bins_d, scaling_k = embedding
    first_bin_size = get_first_bin_size_for_embedding(embedding)

    log.debug(f"{first_bin_size=}")
    ...
    log.debug(f"{median_number_of_spikes_per_bin=}")
    log.debug(f"{number_of_bins_d=}")

    symbol_counts = Counter()

    for raw_symbol in raw_symbols:
        symbol_array = [
            int(raw_symbol[i] > median_number_of_spikes_per_bin[i])
